...
app = FastAPI(title="Student Fee Management System")

# Initialize database spreadsheet at boot
init_db()

# Route Static files & Jinja layouts
...
async def add_student_view(request: Request, status: str = None, message: str = None):
    # Pass an empty student dict by default so Jinja doesn't crash on GET requests
...
async def handle_add_or_update(
    ...
    # Pack the submitted data immediately so we can return it if validation fails
    submitted_student_data = {
        "Roll_No": Roll_No, "Register_No": Register_No, "Student_Name": Student_Name,
        "Father_Name": Father_Name, "Class": Class, "Course": Course,
    ...
    # Helper function to return the template immediately with current form inputs preserved
    def reject_with_data(error_msg: str):
        return templates.TemplateResponse(
            ...
            context={
                "status": "error",
                "message": error_msg,
                "student": submitted_student_data  # Preserves all text input boxes!
            }
        )

    try:
        # --- Strict Multi-Field Input Validation Engine ---
        
        # 1. Roll Number Validation
        if not Roll_No.strip().isdigit():
            return reject_with_data("Roll Number must contain numbers only.")
            
        # 2. Register Number Validation
        if not Register_No.strip().isdigit():
            return reject_with_data("Register Number must contain numbers only.")
        
        # 3. Student Name Validation
        if not re.match(r"^[A-Za-z\s\.\-\']+$", Student_Name):
            return reject_with_data("Student Name must contain letters, spaces, or valid name characters only.")

        # 4. Fee Year Parameters Validation
        fee_inputs = [
        ...
        for fee_label, fee_value in fee_inputs:
            if not fee_value.strip().isdigit():
                return reject_with_data(f"{fee_label} must be a valid whole number.")

        # Consolidate stripped form data after validation passes
        ...
        if action == "add":
            ...
            if success:
                ...
                # On ultimate success, clear the form by redirecting fresh
                return RedirectResponse(url=f"/add-student?status={status}&message={quote_plus(msg)}", status_code=303)
            else:
                return reject_with_data("Student already exists with this Roll Number. Click Update.")
            ...
            return reject_with_data("Invalid form operation selected.")
...
async def handle_delete_student(request: Request, del_roll_no: str = Form(...)):
    try:
        ...
        elif not del_roll_no.strip().isdigit():
            status, msg = "error", "Roll number must be a number."
        else:
...
async def show_student_view(request: Request, roll_search: str = None, status: str = None, message: str = None):
    student_info = None
